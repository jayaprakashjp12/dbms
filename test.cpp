...
void fun(int ro,string namee,vector<string>a,vector<string>b)
{
  vector<string>name;
...
{
  spi[helper(sem[i])]+=stoi(credits[i])*stoi(b[i]);
  cre[helper(credits[i])]+=stoi(credits[i]);
}
for(int i=1;i<=8;i++)
{
  int c=0;
...
for(int i=0;i<stud_roll.size();i++)
{
 ...
 fin.close();
  fun(stud_roll[i],names[i],course,grade);
